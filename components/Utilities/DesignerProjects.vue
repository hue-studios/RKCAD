...
<template>
	<div class="w-full mt-12 lg:mt-0 lg:w-1/2 lg:ml-2 relative featured-projects">
		<p class="uppercase absolute z-50 -top-[20px] left-0">
			Some of {{ getFirst(name) }}'s favorite projects
		</p>
		<div
			class="w-full flex items-start justify-start flex-row gap-2 snap-x snap-mandatory overflow-x-auto featured-projects__container"
		>
			<nuxt-link
			...
			>
				<img
					:src="imageUrl + project.image + '?key=small'"
					alt="RKC Team"
					class="h-full w-auto featured-projects__project-image"
				/>
				<h2
					v-if="project.project.title"
					class="w-full featured-projects__project-title"
				>
					{{ removeFirst(project.project.title) }}
				</h2>
			</nuxt-link>
		</div>
	</div>
